...
void userInput(UserAction_t action, bool hold [[maybe_unused]]) {
  if (action == Start) {
    ...
    if (Snake::get_snake_info()->get_state() == START)
      Snake::get_snake_info()->set_signal(ENTER);
  } else if (action == Pause)
    Snake::get_snake_info()->set_pause(true);
  else if (action == Terminate)
    Snake::get_snake_info()->set_signal(ESCAPE);
  else if (Snake::get_snake_info()->get_state() == ROTATE)
    Snake::get_snake_info()->set_signal((snake_signal_t)(action - 2));
...
GameInfo_t updateCurrentState() {
  Snake::get_snake_info()->process_signal();
  if (Snake::get_snake_info()->get_state() != EXIT)
    Snake::get_snake_info()->update_game_info();
...
bool Coordinate::out_of_field(int left, int right, int up, int down) const {
...
Snake::~Snake() {
  ...
  for (int i = 0; i < FIELD_ROWS; i++) delete game_info.field[i];
  delete game_info.field;

  for (int i = 0; i < NEXT_ROWS; i++) delete game_info.next[i];
...
void Snake::process_signal() {
  action fsm_matrix[7][7] = {
      {&Snake::exitstate, nullptr, nullptr, nullptr, nullptr, &Snake::spawn,
       nullptr},
      {&Snake::spawn, &Snake::spawn, &Snake::spawn, &Snake::spawn,
       &Snake::spawn, &Snake::spawn, &Snake::spawn},
      {&Snake::exitstate, &Snake::dir_left, &Snake::dir_right, &Snake::dir_up,
       &Snake::dir_down, &Snake::move, &Snake::check_timer},
      {&Snake::move, &Snake::move, &Snake::move, &Snake::move, &Snake::move,
       &Snake::move, &Snake::move},
      {&Snake::grow, &Snake::grow, &Snake::grow, &Snake::grow, &Snake::grow,
       &Snake::grow, &Snake::grow},
      {&Snake::exitstate, &Snake::gameover, &Snake::gameover, &Snake::gameover,
       &Snake::gameover, &Snake::gameover, &Snake::gameover},
      {&Snake::exitstate, &Snake::exitstate, &Snake::exitstate,
       &Snake::exitstate, &Snake::exitstate, &Snake::exitstate,
       &Snake::exitstate}};
  ...
  signal = state == SPAWN || state == MOVE || state == GROW ? signal : NOSIG;
}
...
void Snake::update_game_info() {
  ...
  for (const auto& chain : snake_body) game_info.field[chain.y][chain.x] = 1;
...
void Snake::spawn() {
  ...
  while (cell_number && ++i < FIELD_ROWS) {
    ...
    while (cell_number && ++j < FIELD_COLS)
      if (find(snake_body.begin(), snake_body.end(), Coordinate(j, i)) ==
          snake_body.end())
        --cell_number;
  }
...
void Snake::move() {
  ...
  Coordinate head = curr_dir == left    ? snake_body.front().left()
                    : curr_dir == right ? snake_body.front().right()
                    : curr_dir == up    ? snake_body.front().up()
                                        : snake_body.front().down();

  if (find(snake_body.begin(), snake_body.end(), head) != snake_body.end() ||
      head.out_of_field(0, FIELD_COLS - 1, 0, FIELD_ROWS - 1))
    state = GAMEOVER;
  else {
...
void Snake::grow() {
  ...
  if (snake_body.size() == 200)
    state = GAMEOVER;
  else {
    move();
    state = SPAWN;
    distrib = uniform_int_distribution<>(
        0, FIELD_ROWS * FIELD_COLS - snake_body.size() - 1);
  }
}
...
void Snake::read_highscore() {
  ...
  if (File) {
    File.read(reinterpret_cast<char*>(&(game_info.high_score)),
...
void Snake::write_highscore() const {
  ...
  if (File) {
    File.write(reinterpret_cast<const char*>(&(game_info.high_score)),
               sizeof(game_info.high_score));
    File.close();
  }
}