...
GameWindow::GameWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    setStyleSheet("background-color: rgb(250, 215, 244);");
...
void GameWindow::paintEvent(QPaintEvent *event) {
    ...
    for (int i = 0; i < FIELD_ROWS; ++i)
        for (int j = 0; j < FIELD_COLS; ++j) {
            ...
            painter.setBrush(game_info.field[i][j] ? Qt::blue : QColor(250, 215, 244));
    ...
    painter.drawText(infoFieldX + 125, y, QString("%1").arg(game_info.score, 11, 10, QChar(' ')));
    y += 30;

    painter.drawText(infoFieldX, y, QString("HIGHSCORE"));
    painter.drawText(infoFieldX + 125, y, QString("%1").arg(game_info.high_score, 11, 10, QChar(' ')));
    y += 60;
    ...
    for (int i = 0; i < NEXT_ROWS; ++i)
        for (int j = 0; j < NEXT_COLS; ++j) {
            QRect square(FIELD_COLS * squareSize + 90 + j * squareSize, y + i * squareSize, squareSize, squareSize);
            painter.setBrush(game_info.next[i][j] ? Qt::blue : QColor(250, 215, 244));
            painter.drawRect(square);
        }
    y += 90;

    painter.drawText(infoFieldX, y, QString("LEVEL"));
    painter.drawText(infoFieldX + 125, y, QString("%1").arg(game_info.level, 11, 10, QChar(' ')));
    y += 30;

    painter.drawText(infoFieldX, y, QString("SPEED"));
    painter.drawText(infoFieldX + 125, y, QString("%1").arg(game_info.speed, 11, 10, QChar(' ')));
    y += 30;

    painter.drawText(infoFieldX, y, QString("SPEED UP"));
    painter.drawText(infoFieldX + 125, y, QString("%1").arg((game_info.speed < 10 ? ((game_info.score % 600) * 100) / 600 : 100), 11, 10, QChar(' ')));
    y += 30;
    ...
    painter.setPen(game_info.pause ? QColor() : QColor(255, 171, 241));
...
void s21::Controller::process_user_input(int key_event, bool hold) { // update model after userInput
